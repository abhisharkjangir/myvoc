...
export class Quiz extends Component {
  ...
  fetchQuizWords(){
    ...
    //  coloredWords.push({word : 'Done',color : `linear-gradient(to right, ${hex2rgba(randomColorGenerator(),0.7)}, ${ hex2rgba(randomColorGenerator(),0.7)})`})
          ...
          <div className="row">
            <div className="col-md-12">
              {!this.state.isLoading && this.state.quizwords.length == 0 &&  <div className="quiz-intro animate-left">
                <h3>How Strong is Your Vocabulary?</h3>
                ...
                <button className="button button--size-s button--saqui button--inverted button--text-upper button--text-small button--round-s" data-text="Start the Quiz" onClick={this.fetchQuizWords}>Start the Quiz <i className="fa fa-long-arrow-right"></i></button>
              </div>}
                      ...
                      <div className="quiz-res">
                        <div className="quiz-res__item">
                          <div style={{ background : `linear-gradient(to right, ${hex2rgba(randomColorGenerator(),0.7)}, ${hex2rgba(randomColorGenerator(),0.7)})`}}>
                            {/* <p>Yes : {this.state.wordYouKnow}</p>
                            <p>No : {this.state.wordYouDontKnow}</p> */}
                              <p>You scored {this.state.wordYouKnow/this.state.quizwords.length*100}%</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    <button className="button button--size-s button--saqui button--inverted button--text-upper button--text-small button--round-s" data-text="Replay">
